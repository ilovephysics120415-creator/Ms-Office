...
Office.onReady((info) => {
  if (info.host === Office.HostType.Word) {
    if (document.readyState === "loading") {
      document.addEventListener("DOMContentLoaded", initMediumOption);
    } else {
      initMediumOption();
    }
  }
...
function initMediumOption() {
  const btn = document.getElementById("btn-mcq-medium-option");
  if (btn) {
    btn.addEventListener("click", createMediumOptionTable);
  }
}

async function createMediumOptionTable() {
  await Word.run(async (context) => {
    ...
    const labelColWidthTwips = Math.round(0.7 * 566.929); // Column 2 & Column 4 = 0.7 cm

    // Column 3 & Column 5 = [table width - col1 width - 2 * (col2 width)] / 2
    const contentColWidthTwips = Math.round((totalTableWidthTwips - col0WidthTwips - (2 * labelColWidthTwips)) / 2);
    ...
    // 5. Construct OOXML for 2-row, 5-column table
    const ooxml = `
...
await context.sync();
    updateStatus("Inserted Medium Option table successfully!");
  }).catch((error) => {
    updateStatus("Error inserting Medium Option table: " + error.message);
    console.error("Error inserting Medium Option table: ", error);
  });
}